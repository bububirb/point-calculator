...
var label_size := 12
var label_offset := Vector2(-64.0, -24.0)
...
func _process(_delta):
	...
	# Axis scaling debug
#	if points.size() >= 2:
#		points[1] = ceil(points[1] * Vector2(0, 1.02))
	queue_redraw()
...
func draw_points():
	...
	for point in points:
		...
		if point.y > max_y:
			max_y = point.y
	var vertical_size = pow(10.0, ceilf(log((max_y - min_y) / 10.0) / log(10.0)))
	...
	var spacing = (right - left) / subdivisions_x
	for i in points.size():
		...
		draw_circle(location, point_radius, point_color)
